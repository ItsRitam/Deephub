...
const ServiceCard = ({index, service}) => {
    ...
    const divRef = useRef(null)      //Hook uses here = useRef Hook

    const handleMouseMove = (e)=>{
        const bouns = divRef.current.getBoundingClientRect();
        setPosition({x : e.clientX - bouns.left, y : e.clientY - bouns.top})
    }
  ...
  return (
    ...
    className='relative overflow-hidden max-w-lg m-2 sm:m-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl shadow-gray-100 dark:shadow-white/10' onMouseEnter={()=>setVisible(true)} onMouseLeave={()=>setVisible(false)} ref={divRef} onMouseMove={handleMouseMove}>
         ...
         <div className={`pointer-events-none blur-2xl rounded-full bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 w-[300px] h-[300px] absolute z-0 transition-opacity duration-500 mix-blend-lighten ${visible ? 'opacity-70' : 'opacity-0'}`} style={{top : position.y -150, left : position.x - 150}}/>          {/*   <- self closing div....       */}
