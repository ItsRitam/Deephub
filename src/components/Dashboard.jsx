...
const Dashboard = () => {
  ...
  const fakesCount = recentScans.filter((scan) => scan.prediction === "Fake").length;
  const realsCount = recentScans.filter((scan) => scan.prediction === "Real").length;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-black py-10 px-4 sm:px-12 lg:px-24 xl:px-40 transition-colors duration-300 relative">
        ...
        <div className="relative group shrink-0">
          ...
          >
            {avatarUrl ? (
              ...
              <User className="w-14 h-14 sm:w-16 sm:h-16 text-[#5044E5] dark:text-gray-400" strokeWidth={1.5} />
            )}
      ...
      >
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-2xl shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-gray-900 dark:text-white">{totalScans}</h3>
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-2xl shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Fakes Detected</p>
              <h3 className="text-3xl font-bold text-gray-900 dark:text-white">{fakesCount}</h3>
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-2xl shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Authentic Media</p>
              <h3 className="text-3xl font-bold text-gray-900 dark:text-white">{realsCount}</h3>
            </div>
                      ...
                      >
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            {scan.media_type === "image" ? (
                              <img 
                                src={scan.file_url} 
                                alt="thumbnail" 
                                onClick={() => setSelectedMedia({ url: scan.file_url, type: "Image" })}
                              ...
                              <div 
                                onClick={() => setSelectedMedia({ url: scan.file_url, type: "Video" })}
                              ...
                              >
                                <video 
                                  ...
                                  playsInline
                                />
                                <FileVideo size={16} className="relative text-white z-20 drop-shadow-md" />
                              </div>
                            )}
                            {/* ----------------------------- */}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2 text-gray-600 dark:text-gray-300 capitalize">
                            {scan.media_type === 'video' ? <FileVideo size={16} className="text-red-500"/> : <FileImage size={16} className="text-blue-500"/>}
                            {scan.media_type}
                          </div>
                        </td>
